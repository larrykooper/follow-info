...
module Twitcon
  class User < BasicUser
    ...
    def status
      @status ||= Twitcon::Status.fetch_or_new(@attrs.dup[:status].merge(:user => @attrs.except(:status))) unless @attrs[:status].nil?
    end
